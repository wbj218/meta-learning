...
count[c] += 1
print(c)
train_next = em.Dataset(train[c].data[batch:batch+batch], train[0].target[batch:batch+batch])
...
acc[c] = (acc[c]*(count[c]-1)+test_accs[-1])/count[c]
print(acc)

te = []
te.append(test_accs[-1])

for k in range(2*batch, size, batch):
    i += 1
    temp = 0
    c = 0
    for j in range(5):
    ...
    train_next = em.Dataset(train[c].data[k:k + batch], train[c].target[k:k + batch])
    tr_supplier = train_next.create_supplier(x, y)
