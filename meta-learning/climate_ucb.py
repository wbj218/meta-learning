...
if __name__=="__main__":
    ...
    for i in range(numSet):
        ...
        train_next = em.Dataset(train[next].data[0:batch], train[next].target[0:batch])
        ...
        acc.append(val_accs[-1])

    i = 5
    factor = 5
    tf.global_variables_initializer().run() 
    for i in range(batch, numData, batch):
        i += 1
        temp = 0
        c = 0
        for j in range(numSet):
            e = acc[j] + np.sqrt((factor * np.log(i)) / (count[j]))
            if e > temp:
                temp = e
                c = j
        count[c] += 1
        ...
        train_next = em.Dataset(train[next].data[i:i + 1], train[next].target[i:i + 1])
        tr_supplier = train_next.create_supplier(x, y)
        tr_accs, val_accs, test_accs = [], [], []
        ...
        acc[c] = (acc[c] * (count[c] - 1) + val_accs[-1]) / count[c]
