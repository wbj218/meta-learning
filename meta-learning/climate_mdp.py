...
def toDeci(str, base):
    ...
    for i in range(llen - 1, -1, -1):
        ...
        if return_val(str[i]) >= base:
            print('Invalid Number')
            return -1
        num += return_val(str[i]) * power
        power = power * base
    return num
...
if __name__=="__main__":
    ...
    chain, count = [], []
    for i in range(numSet):
        ch, co = markov.get_chain(train[i].target, n_class)
        chain.append(ch)
        count.append(co)
    ...
    P_a = []
    for i in range(numSet):
        result = np.kron(p[i], np.identity(n_class))
        for j in range(numSet - 2):
            result = np.kron(result, np.identity(n_class))
        P_a.append(result)

    A_ub = np.zeros(shape=[n_class ** numSet * numSet, n_class ** numSet])
    b_ub = np.zeros(n_class ** numSet * numSet)
    c = np.ones(n_class ** numSet)

    # solve MDP linear programming
    for i in range(n_class ** numSet):
        seq = fromDeci(n_class, i)
        for j in range(numSet):
            row = numSet * i + j
            A_ub[row, i] = 1-gamma*P_a[j][i, i]
            for m in range(n_class ** numSet):
                if m != i:
                    A_ub[row, m] = -gamma * P_a[j][i, m]
            A_ub[row] = -A_ub[row]
            label_on_arm = int(seq[numSet - 1 - j])
            b_ub[row] = reward[j][label_on_arm]
    ...
    percentage = np.zeros((numSet, n_class))

    for i in range(0, numData, batch):
        label_at_i= ""
        for j in range(numSet):
            label_at_i += str(chain[numSet-1-j][i])
        state= toDeci(label_at_i, n_class)
        V_i= V[state]
        ...
        train_next = em.Dataset(train[action_i].data[i:i + batch], train[action_i].target[i:i + batch])
        tr_supplier = train_next.create_supplier(x, y)
        ...
        #     yyy[y_true_label == ii] = 1
        #     for jj in range(len(y_test_label)):
        #         if y_true_label[jj] == ii and y_test_label[jj] == ii:
        #             ccount += 1
        #     print("class " + str(ii) + " count: ", sum(yyy))
        #     print("class " + str(ii) + " accuracy: ", ccount / sum(yyy))
        ...
        validation_accuracy.append(val_accs[-1])
        test_accuracy.append(test_accs[-1])
