...
def semeion(data_root_folder=None, one_hot=True, partitions=None, shuffle=False, seed=None):
    """
    ...
    datasets = Digit.read_semeion()
    train = em.Dataset(datasets.train.images, datasets.train.labels, name="semeion")
    validation = em.Dataset(datasets.validation.images, datasets.validation.labels, name="semeion")
    test = em.Dataset(datasets.test.images, datasets.test.labels, name="semeion")
    res = [train, validation, test]
...
def opt(data_root_folder=None, one_hot=True, partitions=None, shuffle=False, seed=None):
    """
    data_folder_name = 'mnist'
    ...
    train = em.Dataset(datasets.train.images, datasets.train.labels, name="opt")
    validation = em.Dataset(datasets.validation.images, datasets.validation.labels, name="opt")
...
def meta_omniglot(folder=OMNIGLOT_RESIZED, std_num_classes=None, std_num_examples=None,
                  one_hot_enc=True, _rand=0, n_splits=None, flatten= False):
    """
            ...
            for c in _cls:
                ...
                for img_name in all_images:
                    ...
                    for rot in self._rotations:
                        ...
                        if flatten:
                            self._loaded_images[c + os.path.sep + 'rot_' + str(rot)][img_name] = img.reshape(-1)
                        else:
    ...
    n_splits = n_splits or (0, 100, 200, 300)
